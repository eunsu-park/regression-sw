...
#   ./validation.sh                                  # Run all io × model combos
#   ./validation.sh --filter out12h                  # Only io configs matching "out12h"
#   ./validation.sh --model transformer              # Only transformer model
#   ./validation.sh --filter in2d --model gnn_tcn    # Specific io + model
...
#   ./validation.sh --config-name dev                # Use configs/dev.yaml (default: local)
#
# Usage (file-based):
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: ./validation.sh [--config-file FILE] [--max-jobs N] [--filter PATTERN] [--model MODEL] [--dry-run] [--epoch EPOCH] [--config-name NAME]"
            exit 1
            ;;
    esac
done
